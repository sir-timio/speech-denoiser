...
def get_train_val_test_loaders(config):
    ...
    if 'test' in config:
        test_set = AudioSet(file_path=config['test']['ann_path'],
                            sample_rate=config['sample_rate'],
                            sample_len=config['sample_len'],
                            mode='test',
                            shift=config['shift'],
...
class AudioSet(Dataset):
    ...
    def __getitem__(self, i):
        ...
        noisy, _ = librosa.load(
            os.path.abspath(os.path.expanduser(noisy_path)), sr=self.sr,
        )

        clean, _ = librosa.load(
            os.path.abspath(os.path.expanduser(clean_path)), sr=self.sr,
        )
